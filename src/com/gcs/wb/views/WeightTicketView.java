...
//                    txtGRText.setEnabled(false);
                ...
                if (cbxSLoc.getSelectedIndex() > -1
                    ...
                    if (weightTicket.getCharg() != null && !weightTicket.getCharg().trim().isEmpty()) {
                        batch = weightTicketController.findByWerksLgortMatnrCharg(config.getwPlant(), lgort, weightTicketDetail.getMatnrRef(), weightTicket.getCharg());
                    } else if (!isWithoutDO() && outbDel.getCharg() != null && !outbDel.getCharg().trim().isEmpty()) {
                        batch = weightTicketController.findByWerksLgortMatnrCharg(config.getwPlant(), lgort, weightTicketDetail.getMatnrRef(), outbDel.getCharg());
                    }
